...
public class CarDealershipSimulator2 {
  ...
  public static void main(String[] args)
  {
		...
		while (scan.hasNextLine()){
			...
			String[] input = command.split("[^a-zA-Z0-9]+");
			...
			if((!choice.equals("MENU"))&&(!choice.equals("L"))&&(!choice.equals("BUY"))&&(!choice.equals("RET"))&&(!choice.equals("ADD"))&&(!choice.equals("SPR"))&&(!choice.equals("SSR"))&&(!choice.equals("FEL"))&&(!choice.equals("FAW"))&&(!choice.equals("SMR"))&&(!choice.equals("FCL"))&&(!choice.equals("Q"))&&(!choice.equals("SALES"))&&(!choice.equals("MENU"))&&(!choice.equals("FPR"))){
				System.out.println("Sorry I didn't understand that, try a different command.1");
			}

			else if(choice.equals("L")){
				if(dealer.getCars().isEmpty()){
					System.out.println("Sorry, there are currently no vehicles in the lot. Add some by using the 'L' command.");
				}
				else{
				...
				if(count==0){
						System.out.println("No vehicle with that VIN could be found. Please try again.");
				...
				if(dealer.getAccounts().getTransactions().isEmpty()){
					System.out.println("Sorry, no cars have been bought yet.");
				}
				else if(count2==0){
		...
		if(input.length>1){
			...
			int nuChoice;
			if((!choice.equals("TEAM"))&&(!choice.equals("TOPSP"))&&(!choice.equals("STATS"))){
				try{
					nuChoice = Integer.parseInt(choice);
					if(nuChoice>11){
					...
					System.out.println("The sales for the month of "+date.getDisplayName(Calendar.MONTH, Calendar.SHORT_FORMAT, Locale.CANADA)
									+ " are:");
					int count=0;
					for(Map.Entry<Integer,Transaction> entry: dealer.getAccounts().getTransactions().entrySet()){
